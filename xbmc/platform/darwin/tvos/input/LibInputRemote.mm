...
- (void)startSiriRemoteIdleTimer
{
  ...
  if (m_siriRemoteIdleTimer != nil)
    [self stoptSiriRemoteIdleTimer];
  if (g_xbmcController.inputHandler.inputSettings.siriRemoteIdleTimer)
  {
    auto fireDate = [NSDate
        dateWithTimeIntervalSinceNow:g_xbmcController.inputHandler.inputSettings.siriRemoteIdleTime];
...
- (void)stoptSiriRemoteIdleTimer
{
  ...
  m_siriRemoteIdleState = NO;
}

- (void)setSiriRemoteIdleState
{
  m_siriRemoteIdleState = YES;
}
