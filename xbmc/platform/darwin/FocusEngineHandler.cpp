...
static std::atomic_flag sg_focusenginehandler_lock = ATOMIC_FLAG_INIT;
  ...
  if (m_focus.itemFocus)
  {
    ...
    switch(m_state)
    {
      ...
      case FocusEngineState::Clear:
      {
        ...
        CRect rect = focus.itemFocus->GetRenderRegion();
      ...
      case FocusEngineState::Update:
      {
        ...
        if (!rect.IsEmpty())
        {
          ...
          // handle control zoom
          if ((focusAnimate.zoomX > 0.0f && focusAnimate.zoomY > 0.0f))
          {
            TiXmlElement node("animation");
            node.SetAttribute("reversible", "false");
...
CFocusEngineHandler::GetFocusRect()
{
  ...
  if (VALID_FOCUS_WINDOW(focus))
  {
    ...
    if (focus.itemFocus)
    {
      CRect focusedRenderRect = focus.itemFocus->GetRenderRegion();
...
void CFocusEngineHandler::UpdateFocus(FocusEngineFocus &focus)
{
  ...
  if (NOT_VALID_FOCUS_WINDOW(focus))
  {
    ...
    focus.windowID = CServiceBroker::GetGUI()->GetWindowManager().GetActiveWindow();
    ...
    if (focus.windowID == WINDOW_FULLSCREEN_LIVETV || focus.windowID == WINDOW_VIDEO_MENU)
      focus.window = CServiceBroker::GetGUI()->GetWindowManager().GetWindow(WINDOW_FULLSCREEN_VIDEO);
    else if (focus.windowID == WINDOW_FULLSCREEN_RADIO)
      focus.window = CServiceBroker::GetGUI()->GetWindowManager().GetWindow(WINDOW_VISUALISATION);
    else
      focus.window = CServiceBroker::GetGUI()->GetWindowManager().GetWindow(focus.windowID);
  ...
  switch(focus.rootFocus->GetControlType())
  {
    ...
    {
      focus.itemFocus = focus.rootFocus->GetParentControl();
    }
      break;
  }
}
...
void CFocusEngineHandler::UpdateFocusability()
{
  ...
  if (VALID_FOCUS_WINDOW(m_focus))
  {
    CRect boundsRect = CRect(0, 0, (float)CServiceBroker::GetWinSystem()->GetGfxContext().GetWidth(), (float)CServiceBroker::GetWinSystem()->GetGfxContext().GetHeight());
